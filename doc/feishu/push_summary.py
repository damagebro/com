...
class Publisher:
    ...
    def ensure_nodes(self):
        for entry in self.entries:
            ...
            elif page.get("parent") != entry.get("parent"):
                raise RuntimeError(f"Chapter parent changed: {key}; move explicitly before syncing")
            elif page["title"] != entry["title"]:
                document = page["document_id"]
                before_revision = self.revision(document)
                if page.get("revision") is not None and before_revision != page["revision"]:
                    raise RuntimeError(f"Remote edits detected before rename: {key}")
                before_body = self.client.items(f"/docx/v1/documents/{document}/blocks")
                self.mutate(f"rename {key}", "POST", f"/wiki/v2/spaces/{self.space}/nodes/{page['node_token']}/update_title",
                            json={"title": entry["title"]})
                after_revision = self.revision(document)
                after_body = self.client.items(f"/docx/v1/documents/{document}/blocks")
                if (after_revision != before_revision + 1 or
                    [b for b in after_body if b["block_id"] != document] !=
                    [b for b in before_body if b["block_id"] != document]):
                    raise RuntimeError(f"Remote body changed during rename: {key}")
                page["title"] = entry["title"]
                page["revision"] = after_revision
                self.persist()
    ...
    def publish_page(self, entry):
        ...
        prepared = prepare_links(content, source, self.args.root, self.state["pages"],
        ...
        rewrite_links(converted["blocks"], source, self.args.root, self.state["pages"],
                      self.args.repository_url, self.warnings)
        images = {}
        image_hashes = {}
...
def main():
    ...
    parser.add_argument("--title", default="COMMON RTL 文档")
    parser.add_argument("--state", type=Path)
    parser.add_argument("--repository-url", default="https://github.com/damagebro/com")
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--dry-run", action="store_true", help="Preview only (default)")
    mode.add_argument("--push", action="store_true", help="Create or update Feishu pages")
