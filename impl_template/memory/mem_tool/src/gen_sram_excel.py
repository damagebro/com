...
def gen_sram_shell( sram_cfg_json ):
    ...
    str_shel_path = str_ancr_path + '/rtl/shell/'
    str_ori_prefix = 'com'
    ...
    for sram_type in g_li_sram_type:
        ...
        if( str_re_find(sram_type,'sprom') ):
            ori_name = f'{str_ori_prefix}_{sram_type}_manual'
            new_name = f'{str_new_prefix}_{sram_type}_manual'
            ...
            if( not os.path.isfile(new_file) ):
                ...
                print( f'skip existing user-edit file: {new_file}' )
...
def cfg_parser():
    'ret sram_cfg_json'
    memory_root = pathlib.Path(__file__).resolve().parents[2]
    sram_cfg_json = {
        "common_opt" : {
            "gen_sram_shell"    : 0,
            "gen_sram_list"     : 0,
    ...
    parser.add_argument('-p', '--subsys_prefix', '--subsys_preifx', dest='subsys_prefix', required=True, help='subsystem prefix, such as cpu, npu or pcie')
    ...
    sram_cfg_json['common_opt']['subsys_prefix'] = args.subsys_prefix
    if( args.mode=='init' ):
    ...
    sram_cfg_json['common_opt']['env_var']['anchor_path'] = str(memory_root)
