...
input  wire               i_rx_ebus_wa_valid   , //ebus data_after_addr+data_with_addr+data_before_addr all ok;
...
output wire [LW-1:0]   o_tx_axi_awlen   , //not reg_out, take little timing;
output wire [UW-1:0]   o_tx_axi_awuser  ,
output wire            o_tx_axi_awvalid , //axi have data_after_addr+data_with_addr;   axi not have data_before_addr;
input  wire            i_tx_axi_awready ,
output wire [DW-1:0]   o_tx_axi_wdata   ,
output wire [DW/8-1:0] o_tx_axi_wstrb   , //not reg_out, take some timing;
output wire            o_tx_axi_wlast   , //not reg_out, take some timing;
output wire            o_tx_axi_wvalid  ,
input  wire            i_tx_axi_wready  ,
input  wire            i_tx_axi_bvalid  ,
output wire            o_tx_axi_bready   //,  //tie 1
);
...
//signal declare-------------------------------------------------------------
wire                ebus_cmd_hs         ; // = i_rx_ebus_wa_valid && o_rx_ebus_wa_ready;
wire                axi_cmd_hs          ; // = o_tx_axi_awvalid && i_tx_axi_awready;
wire                ebus_wd_hs          ; // = i_rx_ebus_wd_valid && o_rx_ebus_wd_ready;
wire                axi_wd_hs           ; // = o_tx_axi_wvalid && i_tx_axi_wready;
...
wire  [2+SW_L2*2+AXI_LW-1:0] u_wafifo_wr_data      ; //= {r_fst_split_flag,b_lst_split_flag,fst_split_addr_lsb,lst_split_addr_lsb,o_tx_axi_awlen};
...
//statement------------------------------------------------------------------
assign ebus_cmd_hs = i_rx_ebus_wa_valid && o_rx_ebus_wa_ready;
assign axi_cmd_hs  = o_tx_axi_awvalid && i_tx_axi_awready;
assign ebus_wd_hs  = i_rx_ebus_wd_valid && o_rx_ebus_wd_ready;
assign axi_wd_hs   = o_tx_axi_wvalid && i_tx_axi_wready;
assign ebus_bytelen_modify = i_rx_ebus_wa_bytelen + i_rx_ebus_wa_addr[SW_L2-1:0]; //assert( ebus_bytelen_modify[EBUS_LW]==1'b0 )

assign o_tx_axi_awaddr = r_addr; //only first split maybe not align to word(DW/8), the next addr all align to word;
assign o_tx_axi_awlen  = fnl_wordlen_m1;
assign o_tx_axi_awuser = r_user;
assign o_tx_axi_awvalid= r_split_busy && b_axi_wa_cmd_avl && !u_wafifo_wr_full && !u_wa2wb_fifo_wr_full;
assign o_tx_axi_wdata = u_wdfifo_rd_data;
assign o_tx_axi_wstrb = w_axi_wstrb;
assign o_tx_axi_wlast = w_axi_wlast;
assign o_tx_axi_wvalid= !u_wdfifo_rd_empty && b_axi_wd_send_avl && (!u_wafifo_rd_empty || b_axi_wa_cmd_avl);
assign o_tx_axi_bready= 1'b1;
assign o_rx_ebus_wa_ready = !r_split_busy || (r_split_busy&&b_lst_split_flag&&axi_cmd_hs);
assign o_rx_ebus_wd_ready = !u_wdfifo_wr_full;
assign o_rx_ebus_wb_valid = i_tx_axi_bvalid && !u_wa2wb_fifo_rd_empty && u_wa2wb_fifo_rd_data==1'b1;
//1. split
...
always @(posedge clk or negedge rst_n)begin
    if( !rst_n )
        ...
        r_addr <= i_rx_ebus_wa_addr;
    else if( axi_cmd_hs && !b_lst_split_flag )
        r_addr <= fnl_addr_nxt;
end
always @(posedge clk)begin
    if( ebus_cmd_hs )
        r_user <= i_rx_ebus_wa_user;
end

//2. wa_fifo, wd_fifo, wa_cnt+wd_cnt, wd_strb+wd_last------
wire [AXI_LW-0:0] tmp_axi_len_p1 = axi_cmd_hs ? (o_tx_axi_awlen+1'b1) : '0;
...
wire              strb_lst_data_flag  = strb_lst_split_flag && o_tx_axi_wlast;
...
wire [AXI_LW-1:0] strb_axi_awlen      = u_wafifo_rd_empty&&b_axi_wa_cmd_avl ? o_tx_axi_awlen : wa2wd_axi_awlen;
...
assign w_axi_wlast = o_tx_axi_wvalid && r_axi_len_cnt>=strb_axi_awlen;
always @(posedge clk or negedge rst_n) begin
    if( !rst_n )
        r_axi_len_cnt  <= '0;
    else if( clear || axi_wd_hs&&o_tx_axi_wlast )
        r_axi_len_cnt  <= '0;
    else if( axi_wd_hs )
        r_axi_len_cnt  <= r_axi_len_cnt+1'b1;
end

assign u_wafifo_wr_en   = axi_cmd_hs && !((r_axi_wa2wd_cnt[BUF_CW]&&o_tx_axi_awlen=='0) || (r_axi_wa2wd_cnt[BUF_CW-1:0]=='0 && axi_wd_hs&&o_tx_axi_wlast));  //r_axi_wa2wd_cnt[BUF_CW] mean r_axi_wa2wd_cnt<0,
assign u_wafifo_wr_data = {r_fst_split_flag,b_lst_split_flag,fst_split_addr_lsb,lst_split_addr_lsb,o_tx_axi_awlen};
assign u_wafifo_rd_en   = axi_wd_hs&&o_tx_axi_wlast && !u_wafifo_rd_empty;
assign u_wdfifo_wr_en   = ebus_wd_hs;
assign u_wdfifo_wr_data = i_rx_ebus_wd_data;
...
(
    ...
    .i_wr_en       ( u_wafifo_wr_en    ),   //i
    .i_wr_data     ( u_wafifo_wr_data  ),   //i
    .o_wr_full     ( u_wafifo_wr_full  ),   //o
    .i_rd_en       ( u_wafifo_rd_en    ),   //i
    .o_rd_data     ( u_wafifo_rd_data  ),   //o
    .o_rd_empty    ( u_wafifo_rd_empty ),   //o
    .o_water_level (                   )  //o
);
com_sync_fifo_reg #(
    .DW    ( DW        ),   //8
...
(
    ...
    .i_wr_en       ( u_wdfifo_wr_en    ),   //i
    .i_wr_data     ( u_wdfifo_wr_data  ),   //i
    .o_wr_full     ( u_wdfifo_wr_full  ),   //o
    .i_rd_en       ( u_wdfifo_rd_en    ),   //i
    .o_rd_data     ( u_wdfifo_rd_data  ),   //o
    .o_rd_empty    ( u_wdfifo_rd_empty ),   //o
    .o_water_level (                   )  //o
);
...
assign u_wa2wb_fifo_rd_en   = i_tx_axi_bvalid && o_tx_axi_bready;
com_sync_fifo_reg #(
    .DW    ( 1         ),   //8
    .DEPTH ( MAX_OSD+0 )  //4
...
(
    ...
    .i_wr_en       ( u_wa2wb_fifo_wr_en    ),   //i
    .i_wr_data     ( u_wa2wb_fifo_wr_data  ),   //i
    .o_wr_full     ( u_wa2wb_fifo_wr_full  ),   //o
    .i_rd_en       ( u_wa2wb_fifo_rd_en    ),   //i
    .o_rd_data     ( u_wa2wb_fifo_rd_data  ),   //o
    .o_rd_empty    ( u_wa2wb_fifo_rd_empty ),   //o
    .o_water_level (                       )  //o
);
...
always @(posedge clk or negedge rst_n) begin
    if( !rst_n )begin
        ...
        if( axi_wd_hs&&o_tx_axi_wlast   ) r_dat_burst_cnt <= r_dat_burst_cnt + 1'b1;
        if( i_tx_axi_bvalid&&o_tx_axi_bready ) r_rsp_burst_cnt <= r_rsp_burst_cnt + 1'b1;
        if( axi_cmd_hs             ) r_cmd_beat_cnt  <= r_cmd_beat_cnt  + o_tx_axi_awlen+1'b1;
