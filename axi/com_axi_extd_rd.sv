...
output wire [1:0]                 o_rdfifo_ram_we_n    ,
...
wire                ebus_cmd_hs           ; // = i_rx_ebus_ra_valid && o_rx_ebus_ra_ready;
wire                axi_cmd_hs            ; // = o_tx_axi_arvalid && i_tx_axi_arready;
wire                axi_rd_hs             ; // = i_tx_axi_rvalid && o_tx_axi_rready;
wire                ebus_rd_hs            ; // = o_rx_ebus_rd_valid && i_rx_ebus_rd_ready;
...
assign ebus_cmd_hs = i_rx_ebus_ra_valid && o_rx_ebus_ra_ready;
assign axi_cmd_hs  = o_tx_axi_arvalid && i_tx_axi_arready;
assign axi_rd_hs   = i_tx_axi_rvalid && o_tx_axi_rready;
assign ebus_rd_hs  = o_rx_ebus_rd_valid && i_rx_ebus_rd_ready;
assign ebus_bytelen_modify = i_rx_ebus_ra_bytelen + i_rx_ebus_ra_addr[SW_L2-1:0]; //assert( ebus_bytelen_modify[EBUS_LW]==1'b0 )

assign o_tx_axi_araddr  = r_addr;
assign o_tx_axi_arlen   = fnl_wordlen_m1;
assign o_tx_axi_aruser  = r_user;
assign o_tx_axi_arvalid = r_split_busy && b_rdbuf_avl && b_rdosd_avl;
assign o_tx_axi_rready = tie_rdbuf_bypass_flag ? i_rx_ebus_rd_ready : !u_rdfifo_wr_full; //assert(!tie_rdbuf_bypass_flag && !u_rdfifo_wr_full); when BUF_DEPTH>0, o_tx_axi_rready===1;
assign o_rx_ebus_ra_ready = !r_split_busy || (r_split_busy&&b_lst_split_flag&&axi_cmd_hs);
assign o_rx_ebus_rd_data  = tie_rdbuf_bypass_flag ? i_tx_axi_rdata : u_rdfifo_rd_data[0 +:DW];
assign o_rx_ebus_rd_last  =(tie_rdbuf_bypass_flag ?(i_tx_axi_rlast && u_rainfo_rd_data==1'b1) : u_rdfifo_rd_data[DW +:1]) && o_rx_ebus_rd_valid;
assign o_rx_ebus_rd_valid = tie_rdbuf_bypass_flag ? i_tx_axi_rvalid : !u_rdfifo_rd_empty;
...
always @(posedge clk or negedge rst_n)begin
    if( !rst_n )
        ...
        r_addr <= i_rx_ebus_ra_addr;
    else if( axi_cmd_hs && !b_lst_split_flag )
        r_addr <= fnl_addr_nxt;
end
always @(posedge clk)begin
    if( ebus_cmd_hs )
        r_user <= i_rx_ebus_ra_user;
end
...
assign b_rdbuf_avl = tie_rdbuf_bypass_flag ? 1'b1 : (rdbuf_avl_num>BUF_CW'(o_tx_axi_arlen));
always @(posedge clk or negedge rst_n)begin
    if( !rst_n )
        r_otf_cnt <= '0;
    else if( clear )
...
if( BUF_DEPTH>0 )begin:gen_rdbuf
    ...
    assign u_rdfifo_wr_data  = {(i_tx_axi_rlast && u_rainfo_rd_data==1'b1), i_tx_axi_rdata};
    ...
    (
        ...
        .i_wr_en       ( u_rdfifo_wr_en       ),   //i
        .i_wr_data     ( u_rdfifo_wr_data     ),   //i
        .o_wr_full     ( u_rdfifo_wr_full     ),   //o
        .i_rd_en       ( u_rdfifo_rd_en       ),   //i
        .o_rd_data     ( u_rdfifo_rd_data     ),   //o
        .o_rd_empty    ( u_rdfifo_rd_empty    ),   //o
        .o_water_level ( u_rdfifo_water_level ),   //o
        .o_ram_ce_n    ( o_rdfifo_ram_ce_n    ),   //o
        .o_ram_we_n    ( o_rdfifo_ram_we_n    ),   //o
        .o_ram_addr    ( o_rdfifo_ram_addr    ),   //o
        .o_ram_wr_data ( o_rdfifo_ram_wr_data ),   //o
        .i_ram_rd_data ( i_rdfifo_ram_rd_data )  //i
    );
end:gen_rdbuf
    ...
    assign o_rdfifo_ram_we_n      = '0;
    assign o_rdfifo_ram_addr    = '0;
    assign o_rdfifo_ram_wr_data = '0;
end:gen_no_rdbuf
endgenerate

//3. rdata resp(i_tx_axi_rlast->ebus_rlast)--
...
assign u_rainfo_rd_en   = axi_rd_hs && i_tx_axi_rlast;
com_sync_fifo_reg #(
    .DW    ( 1             ),   //8
...
(
    ...
    .i_wr_en       ( u_rainfo_wr_en       ),   //i
    .i_wr_data     ( u_rainfo_wr_data     ),   //i
    .o_wr_full     ( u_rainfo_wr_full     ),   //o
    .i_rd_en       ( u_rainfo_rd_en       ),   //i
    .o_rd_data     ( u_rainfo_rd_data     ),   //o
    .o_rd_empty    ( u_rainfo_rd_empty    ),   //o
    .o_water_level ( u_rainfo_water_level )  //o
);
...
`COM_SIGNAL_ASSERT( a2, clk,rst_n,i_tx_axi_rvalid,(tie_rdbuf_bypass_flag || o_tx_axi_rready==1'b1) , "when BUF_DEPTH>0, o_tx_axi_rready===1" );
...
always @(posedge clk or negedge rst_n) begin
    if( !rst_n )begin
        ...
        if( axi_rd_hs&&i_tx_axi_rlast   ) r_dat_burst_cnt <= r_dat_burst_cnt + 1'b1;
        if( axi_cmd_hs             ) r_cmd_beat_cnt  <= r_cmd_beat_cnt  + o_tx_axi_arlen+1'b1;
