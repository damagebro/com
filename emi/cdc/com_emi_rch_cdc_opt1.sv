/******************************************************************************
*
*  Authors:   dmg
*    Email:   dmg@sensetime.com
*     Date:   2022/06/22-11:24:08
*
*  Description:
*  -opt1: use tpram2ck to deal cdc
*  -opt2: use async_fifo_reg to deal cdc
*  -recommend opt1
*
*  Modify:
*  -
*
******************************************************************************/

`ifndef com_emi_rch_cdc_opt1_v
`define com_emi_rch_cdc_opt1_v
module com_emi_rch_cdc_opt1 #( parameter
    AW      = 32        , //emi bus addr bit_width
    DW      = 128       , //emi bus data bit_width
    RCH     = 4         , //number of read channel
    MAX_RCH = 16        , //number of max read channels
    MAX_OSD = 16        , //number of max outstanding
    MAX_LEN = 16        , //number of max burst len
    USR_W   = 0         , //emi bus user signal bit_width, typical value=0; maybe used by cache control, and any DIY functions
    BOUND_BYTES = 4096  , //4k boundary split required by interleave dram bank; modify this value must be 2^n, typical value is (512, 1024, 2048, 4096)
    STR_LOG_PREFIX = "" ,

    UW =(USR_W>0?USR_W:1),
    SW = DW/8            ,
    IW = $clog2(MAX_RCH) ,

    RAM_DEPTH = 16, //read channel data ram depth, typical value is 16,  available vaule is [2,MAX_OSD*MAX_LEN] \
                   //recommend use the default value, for user send read request, must make sure read data can by received, \
                   //so emi bus don't need storaged read data \
                   //when RAM_DEPTH==0, the spram_if(cen,we..) don't need connect to sram;
    RAM_AW = $clog2(RAM_DEPTH>2?RAM_DEPTH:2),
    RAM_DW = USR_W + 1 + IW + DW//, //{user,rlast,rid,data}
)
(
input  wire                     clk                 ,
input  wire                     rst_n               ,
input  wire                     clear               ,
input  wire                     axi_clk             ,
input  wire                     axi_rst_n           ,
input  wire                     axi_clear           ,
//cfg&status---
input  wire [7:0]               max_burst_len       , //emi clk doamin
output wire                     clr_ongoing         , //emi clk doamin
//sram_if---
output wire                     emi_rch_ram_wr_en   ,
output wire [RAM_AW-1:0]        emi_rch_ram_wr_addr ,
output wire [RAM_DW-1:0]        emi_rch_ram_wr_data ,
output wire                     emi_rch_ram_rd_en   ,
output wire [RAM_AW-1:0]        emi_rch_ram_rd_addr ,
input  wire [RAM_DW-1:0]        emi_rch_ram_rd_data ,
//dp---
com_emi_if.usr_rch_rx           usr_emi_ifs[RCH-1:0], //emi clk domain
com_emi_if.ext_rch_tx           ext_emi_ifm         //, //axi clk domain
);
//localparam-----------------------------------------------------------------
//reg  declare---------------------------------------------------------------
//wire declare---------------------------------------------------------------
//statement------------------------------------------------------------------

//arb---
wire          arb_arvalid  ;
wire          arb_arready  ;
wire [IW-1:0] arb_arid     ;
wire [AW-1:0] arb_araddr   ;
wire [7:0]    arb_arlen    ;
wire [UW-1:0] arb_aruser   ;
wire          arb_rvalid   ;
wire          arb_rready   ;
wire [IW-1:0] arb_rid      ;
wire [DW-1:0] arb_rdata    ;
wire          arb_rlast    ;
wire [UW-1:0] arb_ruser    ;
com_emi_rch_arb #(
    .AW         ( AW         ), //32
    .DW         ( DW         ), //128
    .RCH        ( RCH        ), //4
    .MAX_RCH    ( MAX_RCH    ), //16
    .USR_W      ( USR_W      )  //0
)u_com_emi_rch_arb
(
    .clk                  ( clk                  ), //i
    .rst_n                ( rst_n                ), //i
    .clear                ( clear                ), //i

    .usr_emi_ifs          ( usr_emi_ifs          ), //if

    .tx_arvalid           ( arb_arvalid          ), //o
    .tx_arready           ( arb_arready          ), //i
    .tx_arid              ( arb_arid             ), //o
    .tx_araddr            ( arb_araddr           ), //o
    .tx_arlen             ( arb_arlen            ), //o
    .tx_aruser            ( arb_aruser           ), //o

    .tx_rvalid            ( arb_rvalid           ), //i
    .tx_rready            ( arb_rready           ), //o
    .tx_rid               ( arb_rid              ), //i
    .tx_rdata             ( arb_rdata            ), //i
    .tx_rlast             ( arb_rlast            ), //i
    .tx_ruser             ( arb_ruser            )  //i
);

//split&dp_fifo_ram---
//dp fifo ram-
wire           dp_rx_rvalid   ; //axi clk domain
wire           dp_rx_rready   ;
wire [IW-1:0]  dp_rx_rid      ;
wire [DW-1:0]  dp_rx_rdata    ;
wire           dp_rx_rlast    ;
wire [UW-1:0]  dp_rx_ruser    ;
wire           dp_tx_rvalid   ; //emi clk domain
wire           dp_tx_rready   ;
wire [IW-1:0]  dp_tx_rid      ;
wire [DW-1:0]  dp_tx_rdata    ;
wire           dp_tx_rlast    ;
wire [UW-1:0]  dp_tx_ruser    ;

wire              dp_wr_en    = dp_rx_rvalid && dp_rx_rready;
wire [RAM_DW-1:0] dp_wr_data  ;
wire              dp_wr_full  ;
wire              dp_rd_en    = dp_tx_rvalid && dp_tx_rready;
wire [RAM_DW-1:0] dp_rd_data  ;
wire              dp_rd_empty ;
com_async_fifo_ram_2p2ck #(
    .DW         ( RAM_DW     ), //8
    .RAM_DEPTH  ( RAM_DEPTH  )  //4
)u_com_async_fifo_ram_2p2ck_dp
(
    .wr_clk               ( axi_clk              ), //i
    .wr_rst_n             ( axi_rst_n            ), //i
    .wr_clear             ( axi_clear            ), //i
    .rd_clk               ( clk                  ), //i
    .rd_rst_n             ( rst_n                ), //i
    .rd_clear             ( clear                ), //i

    .wr_en                ( dp_wr_en             ), //i
    .wr_data              ( dp_wr_data           ), //i
    .wr_full              ( dp_wr_full           ), //o
    .rd_en                ( dp_rd_en             ), //i
    .rd_data              ( dp_rd_data           ), //o
    .rd_empty             ( dp_rd_empty          ), //o
    .water_level          (                      ), //o

    .ram_wr_en            ( emi_rch_ram_wr_en    ), //o
    .ram_wr_addr          ( emi_rch_ram_wr_addr  ), //o
    .ram_wr_data          ( emi_rch_ram_wr_data  ), //o
    .ram_rd_en            ( emi_rch_ram_rd_en    ), //o
    .ram_rd_addr          ( emi_rch_ram_rd_addr  ), //o
    .ram_rd_data          ( emi_rch_ram_rd_data  )  //i
);
assign dp_tx_rvalid = !dp_rd_empty;
assign dp_rx_rready = !dp_wr_full;

assign arb_rvalid = dp_tx_rvalid;
assign arb_rid    = dp_tx_rid   ;
assign arb_rdata  = dp_tx_rdata ;
assign arb_rlast  = dp_tx_rlast ;
assign arb_ruser  = dp_tx_ruser ;
assign dp_tx_rready = arb_rready;

wire dp_tx_rlast_t;
assign dp_tx_rlast = dp_tx_rlast_t && dp_tx_rvalid;
generate
  if( USR_W==0 ) begin:gen_no_usr
      assign dp_wr_data = {dp_rx_rlast,dp_rx_rid,dp_rx_rdata};
      assign {dp_tx_rlast_t,dp_tx_rid,dp_tx_rdata} = dp_rd_data;
      assign dp_tx_ruser = UW'(0);
  end
  else begin:gen_with_usr
      assign dp_wr_data = {dp_rx_ruser,dp_rx_rlast,dp_rx_rid,dp_rx_rdata};
      assign {dp_tx_ruser,dp_tx_rlast_t,dp_tx_rid,dp_tx_rdata} = dp_rd_data;
  end
endgenerate

//cdc--
wire [IW-1:0]  dst_arid    ;
wire [AW-1:0]  dst_araddr  ;
wire [7:0]     dst_arlen   ;
wire [UW-1:0]  dst_aruser  ;
wire           dst_arvalid ;
wire           dst_arready ;
com_emi_rch_req_cdc #(
    .AW         ( AW         ), //32
    .DW         ( DW         ), //128
    .MAX_WCH    ( MAX_RCH    ), //16
    .USR_W      ( USR_W      )  //0
)u_com_emi_rch_req_cdc
(
    .clk                  ( clk                  ), //i
    .rst_n                ( rst_n                ), //i
    .clear                ( clear                ), //i
    .axi_clk              ( axi_clk              ), //i
    .axi_rst_n            ( axi_rst_n            ), //i
    .axi_clear            ( axi_clear            ), //i
    //dp---
    .rx_arid              ( arb_arid             ), //i
    .rx_araddr            ( arb_araddr           ), //i
    .rx_arlen             ( arb_arlen            ), //i
    .rx_aruser            ( arb_aruser           ), //i
    .rx_arvalid           ( arb_arvalid          ), //i
    .rx_arready           ( arb_arready          ), //o

    .tx_arid              ( dst_arid             ), //o
    .tx_araddr            ( dst_araddr           ), //o
    .tx_arlen             ( dst_arlen            ), //o
    .tx_aruser            ( dst_aruser           ), //o
    .tx_arvalid           ( dst_arvalid          ), //o
    .tx_arready           ( dst_arready          )  //i
);

//split-
wire [7:0] max_burst_len_dmaxi;
com_cdc_sig#(
    .DATA_W ( 8  )
)zr_com_cdc_sig_burst_len(
    .oclk   ( axi_clk            ), //i
    .orst_n ( axi_rst_n          ), //i
    .idata  ( max_burst_len      ), //i
    .odata  ( max_burst_len_dmaxi)  //o
);

wire           split_arvalid ;
wire           split_arready ;
wire [IW-1:0]  split_arid    ;
wire [AW-1:0]  split_araddr  ;
wire [7:0]     split_arlen   ;
wire [UW-1:0]  split_aruser  ;
wire           split_rvalid  ;
wire           split_rready  ;
wire [IW-1:0]  split_rid     ;
wire           split_rlast   ;
com_emi_rch_split #(
    .AW         ( AW         ), //32
    .DW         ( DW         ), //128
    .RCH        ( RCH        ), //4
    .MAX_RCH    ( MAX_RCH    ), //16
    .MAX_OSD    ( MAX_OSD    ), //16
    .USR_W      ( USR_W      ), //0
    .BOUND_BYTES ( BOUND_BYTES )//, //4096
)u_com_emi_rch_split
(
    .clk                  ( axi_clk              ), //i
    .rst_n                ( axi_rst_n            ), //i
    .clear                ( axi_clear            ), //i
    //cfg---
    .max_burst_len        ( max_burst_len_dmaxi  ), //i
    //dp---
    .rx_arvalid           ( dst_arvalid          ), //i
    .rx_arready           ( dst_arready          ), //o
    .rx_arid              ( dst_arid             ), //i
    .rx_araddr            ( dst_araddr           ), //i
    .rx_arlen             ( dst_arlen            ), //i
    .rx_aruser            ( dst_aruser           ), //i

    .rx_rvalid            ( dp_rx_rvalid         ), //o
    .rx_rready            ( dp_rx_rready         ), //i
    .rx_rid               ( dp_rx_rid            ), //o
    .rx_rlast             ( dp_rx_rlast          ), //o

    .tx_arvalid           ( split_arvalid        ), //o
    .tx_arready           ( split_arready        ), //i
    .tx_arid              ( split_arid           ), //o
    .tx_araddr            ( split_araddr         ), //o
    .tx_arlen             ( split_arlen          ), //o
    .tx_aruser            ( split_aruser         ), //o

    .tx_rvalid            ( split_rvalid         ), //i
    .tx_rready            ( split_rready         ), //o
    .tx_rid               ( split_rid            ), //i
    .tx_rlast             ( split_rlast          )  //i
);


//clr---
wire clr_ongoing_dm_axi;
reg  clear_d;
always @(posedge clk or negedge rst_n)
begin
    if( !rst_n )
        clear_d <= 1'b0;
    else
        clear_d <= clear;
end
wire ps_emi_terminate_dm_emi = clear==1'b1 && clear_d==1'b0;//pulse signal,  posedge pulse;
wire ps_emi_terminate_dm_axi;
com_cdc_pulse zr_com_cdc_pulse_clr(
    .iclk      ( clk          ),  //i
    .irst_n    ( rst_n        ),  //i
    .ipulse    ( ps_emi_terminate_dm_emi ),  //i
    .oclk      ( axi_clk      ),  //i
    .orst_n    ( axi_rst_n    ),  //i
    .opulse    ( ps_emi_terminate_dm_axi )//,//o
);
com_cdc_sig#(
    .DATA_W (1      )
)zr_com_cdc_sig_clr(
    .oclk   (clk                ), //i
    .orst_n (rst_n              ), //i
    .idata  (clr_ongoing_dm_axi ), //i
    .odata  (clr_ongoing        )  //o
);

wire       ps_emi_terminate = clear==1'b1 && clear_d==1'b0;//pulse signal,  posedge pulse;
wire       clr_arvalid ;
wire       clr_arready ;
wire       clr_rvalid  ;
wire       clr_rready  ;
com_emi_rch_clr #(
    .MAX_OSD    ( MAX_OSD    )  //16
)u_com_emi_rch_clr
(
    .clk                  ( axi_clk              ), //i
    .rst_n                ( axi_rst_n            ), //i
    .ps_emi_terminate     ( ps_emi_terminate_dm_axi ), //i
    //cfg&status---
    .clr_ongoing          ( clr_ongoing_dm_axi   ), //o
    //dp---
    .rx_arvalid           ( split_arvalid        ), //i
    .rx_arready           ( split_arready        ), //o
    .rx_arlen             ( split_arlen          ), //i

    .rx_rvalid            ( split_rvalid         ), //o
    .rx_rready            ( split_rready         ), //i

    .tx_arvalid           ( clr_arvalid          ), //o
    .tx_arready           ( clr_arready          ), //i

    .tx_rvalid            ( clr_rvalid           ), //i
    .tx_rready            ( clr_rready           ), //o
    .tx_rlast             ( split_rlast          )  //i
);

//ext---
com_emi_rch_ext #(
    .AW         ( AW         ), //32
    .DW         ( DW         ), //128
    .MAX_RCH    ( MAX_RCH    ), //16
    .MAX_OSD    ( MAX_OSD    ), //16
    .USR_W      ( USR_W      )  //0
)u_com_emi_rch_ext
(
    .clk                  ( axi_clk              ), //i
    .rst_n                ( axi_rst_n            ), //i
    .clear                ( 1'b0                 ), //i, externel path can't be reset by clear;
    //dp---
    .rx_arvalid           ( clr_arvalid          ), //i
    .rx_arready           ( clr_arready          ), //o
    .rx_arid              ( split_arid           ), //i
    .rx_araddr            ( split_araddr         ), //i
    .rx_arlen             ( split_arlen          ), //i
    .rx_aruser            ( split_aruser         ), //i

    .rx_rvalid            ( clr_rvalid           ), //o
    .rx_rready            ( clr_rready           ), //i
    .rx_rid               ( split_rid            ), //o
    .rx_rdata             ( dp_rx_rdata          ), //o
    .rx_rlast             ( split_rlast          ), //o
    .rx_ruser             ( dp_rx_ruser          ), //o

    .ext_emi_ifm          ( ext_emi_ifm          )  //if
);

//dbg---
com_emi_rch_dbg #(
    .AW         ( AW         ), //32
    .DW         ( DW         ), //128
    .RCH        ( RCH        ), //4
    .MAX_RCH    ( MAX_RCH    ), //16
    .MAX_OSD    ( MAX_OSD    ), //16
    .USR_W      ( USR_W      ), //0
    .STR_LOG_PREFIX ( STR_LOG_PREFIX )//, //""
)u_com_emi_rch_dbg
(
    .clk                  ( axi_clk                  ), //i
    .rst_n                ( axi_rst_n                ), //i
    .clear                ( axi_clear                ), //i
    //dp---
    .arvalid              ( ext_emi_ifm.emi_arvalid  ), //i
    .arready              ( ext_emi_ifm.emi_arready  ), //i
    .arid                 ( ext_emi_ifm.emi_arid     ), //i
    .araddr               ( ext_emi_ifm.emi_araddr   ), //i
    .arlen                ( ext_emi_ifm.emi_arlen    ), //i

    .rvalid               ( ext_emi_ifm.emi_rvalid   ), //i
    .rready               ( ext_emi_ifm.emi_rready   ), //i
    .rid                  ( ext_emi_ifm.emi_rid      ), //i
    .rdata                ( ext_emi_ifm.emi_rdata    ), //i
    .rlast                ( ext_emi_ifm.emi_rlast    )  //i
);

//perf---
com_emi_rch_perf #(
    .AW         ( AW         ), //32
    .DW         ( DW         ), //128
    .RCH        ( RCH        ), //4
    .MAX_RCH    ( MAX_RCH    ), //16
    .MAX_OSD    ( MAX_OSD    ), //16
    .USR_W      ( USR_W      ), //0
    .STR_LOG_PREFIX ( STR_LOG_PREFIX )//, //""
)u_com_emi_rch_perf
(
    .clk                  ( axi_clk              ), //i
    .rst_n                ( axi_rst_n            ), //i
    //cfg&status---
    .sw_stat_enable       ( 1'b0                 ), //i
    .sw_stat_start        ( 1'b0                 ), //i
    .sw_stat_done         ( 1'b0                 ), //i
    .sw_stat_clear        ( 1'b0                 ), //i
    .sw_stat_bw_period    ( 32'b0                ), //i
    //dp---
    .arvalid              ( ext_emi_ifm.emi_arvalid ), //i
    .arready              ( ext_emi_ifm.emi_arready ), //i
    .arid                 ( ext_emi_ifm.emi_arid    ), //i
    .arlen                ( ext_emi_ifm.emi_arlen   ), //i

    .rvalid               ( ext_emi_ifm.emi_rvalid  ), //i
    .rready               ( ext_emi_ifm.emi_rready  ), //i
    .rid                  ( ext_emi_ifm.emi_rid     ), //i
    .rlast                ( ext_emi_ifm.emi_rlast   )  //i
);


endmodule //end of com_emi_rch_cdc_opt1
`endif //end of com_emi_rch_cdc_opt1_v

