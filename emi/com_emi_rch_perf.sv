...
*     Date:   2020/11/19-09:36:44
*
*  Description:
*  mersure the emi bus latency and bandwidth
*  -latency  : T_delay = T_d-T_a;  T_a: the addr channel each burst request, T_d: the data channel each burst last transfer;
*  -bandwidth: bw = data_transfer_times/100_0000; every 100_0000 cycles, the number of data channel tranfers;
*
*  Modify:
*  -
*
...
`ifndef com_emi_rch_perf_v
`define com_emi_rch_perf_v
module com_emi_rch_perf #( parameter
    AW      = 32        ,
    DW      = 128       ,
    RCH     = 4         ,
    MAX_RCH = 16        ,
...
wire hw_stat_start = !rcb_perf_stat_flag && (arvalid&&arready);
...
wire        la_wr_en    = (rcb_perf_stat_flag||stat_start) && arvalid && arready;
wire [15:0] la_wr_data  = stat_start ? 16'b0 : rc_la_cnt;
wire        la_wr_full  ;
wire        la_rd_en    = rcb_perf_stat_flag && rvalid && rready && rlast;
...
begin
    if( !rst_n )
        ...
        rc_bw_cnt <= 'b0;
    else if( rcb_perf_stat_flag && rvalid&&rready )
...
`ifdef DUMP_EMI
  ...
  function automatic F_fwrite_latency( int cnt );
      ...
      integer fp;
      string fn = {dump_dir,STR_LOG_PREFIX,"emi_perf_rch_latency.txt"};
  ...
  function automatic F_fwrite_bandwidth( int cnt );
      ...
      integer fp;
      string fn = {dump_dir,"emi_perf_rch_bandwidth.txt"};
...
endmodule //end of com_emi_rch_perf
`endif //end of com_emi_rch_perf_v
