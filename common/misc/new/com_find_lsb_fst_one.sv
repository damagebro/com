...
module com_find_lsb_fst_one #( parameter
    N = 8, //range[1:]
    parameter N_L2 = $clog2(N>2?N:2)
)
(
input  wire [N-1:0]             i_req_dat           ,
...
if( N>1 ) begin:gen_larger1
    assign req_msk[N-1:1] = req_msk[N-2:0] | i_req_dat[N-2:0];  //bit by bit assign
end:gen_larger1
endgenerate
assign w_oht = ~req_msk & i_req_dat;
always @(*)begin
    w_idx = '0;
    for( int i=0; i<N; i++ )
        if( w_oht[i] )
...
assign o_res_none_flag = !(|i_req_dat);
